...
###   This script attempts to download the signature file SHA256SUMS.asc from hellebit.org
...
WORKINGDIR="/tmp/hellebit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://hellebit.org/bin/"
VERSIONPREFIX="hellebit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hellebit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Hellebit Core binary release signing key installed?"
   fi
