Hellebit Core version 0.11.0 is now available from:

  <https://hellebit.org/bin/hellebit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Hellebit Core or other software:
...
For example, add the following to `hellebit.conf`:
...
related to the blockchain in the hellebit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Hellebit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Hellebit Core will 
...
For an example script see [hellebit-submittx](https://github.com/laanwj/hellebit-submittx).
...
- #5334 `cf87536` libhellebitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new hellebit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only HellebitAmountField
- #5753 `0f44672` Add hellebit logo to about screen
...
- #5809 `46bfbe7` Add hellebit-cli man page
...
- #5849 `41259ca` contrib/init/hellebitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Hellebit-Qt.app to Hellebit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for hellebit-tx)
